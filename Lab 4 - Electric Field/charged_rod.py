...
while n < 6:
    z = n*(R/2)
    more_arrows.append(arrow(pos=vector(z,0,0), axis=(0,1,0)))
    n=n+1
...
for thisarrow in (list_of_arrows):  ## take arrows one at a time
    ...
    for charge in list_of_charges: 
        ...
        r = thisarrow.pos - charge.pos
        dE = oofpez * dQ * 1/(mag(r)**2) * norm(r)
...
for thisarrow in more_arrows:
    ...
    for charge in list_of_charges: 
        ...
        Enet = Enet + dE
        # (e) after this code is added, uncomment the following two statements to display Enet
        thisarrow.axis = Enet*0.25
        thisarrow.color = color.cyan


##Enet = vector(0,0,0)
##for charge in list_of_charges: 
##        r = vector(0,0,0.5) - charge.pos
##        dE = oofpez * dQ * 1/(mag(r)**2) * norm(r)
##        Enet = Enet + dE
##print Enet
